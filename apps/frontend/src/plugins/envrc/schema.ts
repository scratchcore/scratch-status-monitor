	...
	for (const [key, varConfig] of Object.entries(config.env)) {
		...
		switch (varType) {
			...
			case "number":
				...
				break;
			case "port":
				schema = z.string().regex(/^\d+$/).refine(
					(val) => {
		...
		if (varConfig.validate) {
			schema = schema.refine(
				...
				{
					message:
						typeof varConfig.validate === "function"
							? "Validation failed"
							: "Invalid value",
				},
			);
 ...
 * 環境変数の説明を取得
 */
export function getEnvDescription<T extends EnvConfig>(
	config: T,
	key: string,
): string {
 ...
 * 必須環境変数のリストを取得
 */
export function getRequiredEnvKeys<T extends EnvConfig>(
	config: T,
): string[] {
